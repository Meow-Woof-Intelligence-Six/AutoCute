...
def get_top_features(df, label, method, is_regression):
    ...
    if method == "mannwhitneyu":
        ...
        if g0.empty or g1.empty:
            return []
        pvals = {
            f: mannwhitneyu(g0[f], g1[f], alternative="two-sided").pvalue
            for f in X.columns
        }
...
train_s = (
    train_df.sample(SUBSAMPLE_SIZE, random_state=42) if SUBSAMPLE_SIZE else train_df
)
valid_s = (
    valid_df.sample(SUBSAMPLE_SIZE, random_state=42) if SUBSAMPLE_SIZE else valid_df
)
...
for task_type, labels in tasks.items():
    ...
    for label in labels:
        ...
        consistent = {}
        methods = (
            ["spearman", "mi", "lgbm"] if is_reg else ["mannwhitneyu", "mi", "lgbm"]
        )
        ...
        for m in methods:
            ...
            print(
                f"  {m}: train={len(tr_top)}, val={len(val_top)}, both={len(intersect)}"
            )
