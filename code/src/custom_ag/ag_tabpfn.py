...
class TabPFNV2Model(AbstractModel):
    ...
    def _fit(
        ...
        **kwargs,
        ...
        # 1. 数据量阈值控制
        max_rows = default_auxiliary_params["max_rows"]
        if len(X) > max_rows:
            print(f"数据量 ({len(X)}) 超过阈值 ({max_rows})，将使用最新的 {max_rows} 条数据进行训练。")
            X = X.tail(max_rows)
            y = y.tail(max_rows)
    ...
    def _get_default_resources(self) -> tuple[int, int]:
        ...
        num_gpus = min(1, ResourceManager.get_gpu_count_torch())
    ...
    def _set_default_params(self):
        default_params = {
            ...
            "fit_mode": "low_memory", # to avoid memory issues
            "memory_saving_mode": True,  # to avoid memory issues
        }
