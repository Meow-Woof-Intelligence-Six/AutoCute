...
class TimeSeriesLagPreprocessor:
    def __init__(self, lag_days: Optional[List[int]] = None):
        if lag_days is None:
            ...
            self.lag_days = [x - 1 for x in self.lag_days[1:]]  # 去掉第一个1天的lag, 因为自己就是。
    ...
    def preprocess(self, df: pd.DataFrame, drop_last_n: int = 2, 
                   ...
                   dropna_shift_targets: bool = True,
...
df_cleaned = preprocessor.preprocess(
    ...
    lag_target_only=True,  # 新增参数，只对target_cols进行lag
    dropna_shift_targets=False, # 不删除shift后的目标列的NaN值
)
